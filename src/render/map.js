...
module.exports = function (mapConfig) {
	const h = mapConfig.wallHeight;
	const s = mapConfig.roomSize * Math.pow(2, mapConfig.gridSize);
	const border = genBorder(
		mapConfig.gridSize + 1,
		mapConfig.wallThickness,
		mapConfig.wallRemoval
	).map((v) => [v[0] * s, v[1] * s]);
	const segments = border.slice(0, -1).map((p, i) => [p, border[i + 1]]);
	const normal = segments
		.map(([[x1, y1], [x2, y2]]) => [Math.sign(y1 - y2), 0, Math.sign(x2 - x1)])
		.flatMap((v) => Array(4).fill(v));
	const position = segments.flat().flatMap(([x, y]) => [[x, 0, y], [x, h, y]]);
	const {getAreaIndex, getGridSegments, getGridParts, placements} = genGrid(segments, mapConfig.gridSize, mapConfig.roomSize);
	...
	const elements = Array(position.length / 4)
		.fill()
